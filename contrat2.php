...
if (isset($_GET["id"])) {
    ...
    $stmt = $conn->prepare("SELECT c.`nom`, c.`prenom`, c.`cin`,c.`cin_date`, c.`adresse`,c.`phone`,r.`id` , r.`date`,r.`heure_debut`,r.`heure_fin`, r.`observation`, r.`prix` FROM `client` c, `reservation` r  WHERE r.`id` =? and r.`id_client` = c.`id`");

...
class Xcrud
{
    ...
    public function button($link = '', $name = '', $icon = '', $class = '', $parameters = array(), $condition = array(),$condition2 = array(),$condition3= array())
    {
        if ($link)
        {
            $this->buttons[] = array(
        ...
        if ($condition && is_array($condition) && count($condition) == 3 && $name)
        {
            ...
            if ($condition2 && is_array($condition2) && count($condition2) == 3 && $name)
            {
                list($field2, $operator2, $value2) = $condition2;
                if ($condition3 && is_array($condition3) && count($condition3) == 3 && $name)
                {
                    list($field3, $operator3, $value3) = $condition3;
    ...
    protected function is_button($name, &$row)
    {
        ...
        {
            if(count($this->grid_restrictions[$name])==9){
                $fdata = $this->_parse_field_names($this->grid_restrictions[$name]['field']);
                $fname = key($fdata);
                $fdata2 = $this->_parse_field_names($this->grid_restrictions[$name]['field2']);
                $fname2 = key($fdata2);
                $fdata3 = $this->_parse_field_names($this->grid_restrictions[$name]['field3']);
                $fname3 = key($fdata3);
                return  ($this->_compare($row[$fname], $this->grid_restrictions[$name]['operator'], $this->replace_text_variables($this->
                grid_restrictions[$name]['value'], $row)) ? true : false)&&( $this->_compare($row[$fname2], $this->grid_restrictions[$name]['operator2'], $this->replace_text_variables($this->
                grid_restrictions[$name]['value2'], $row)) ? true : false)&&( $this->_compare($row[$fname3], $this->grid_restrictions[$name]['operator3'], $this->replace_text_variables($this->
                grid_restrictions[$name]['value3'], $row)) ? true : false);
            }
            elseif(count($this->grid_restrictions[$name])==6){
                $fdata = $this->_parse_field_names($this->grid_restrictions[$name]['field']);
                $fname = key($fdata);
                $fdata2 = $this->_parse_field_names($this->grid_restrictions[$name]['field2']);
                $fname2 = key($fdata2);
              return  ($this->_compare($row[$fname], $this->grid_restrictions[$name]['operator'], $this->replace_text_variables($this->
              grid_restrictions[$name]['value'], $row)) ? true : false)&&( $this->_compare($row[$fname2], $this->grid_restrictions[$name]['operator2'], $this->replace_text_variables($this->
              grid_restrictions[$name]['value2'], $row)) ? true : false);
            }else{
            $fdata = $this->_parse_field_names($this->grid_restrictions[$name]['field']);
            $fname = key($fdata) /*$fdata[0]['table'] . '.' . $fdata[0]['field']*/;
            return $this->_compare($row[$fname], $this->grid_restrictions[$name]['operator'], $this->replace_text_variables($this->
